...
export default function SendOtp({ setOtpSendInputOpen, setOtpInputOpen }) {

    const usernameRef = useRef(null);

    async function onSendOtpSubmit(e) {
        e.preventDefault();
        const username = usernameRef.current.value;

        try {
            const response = await axios.post(
                "http://localhost:7070/sendotp",
                {
                    username: username,
                },
                {
    ...
    return (
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg w-[90%] sm:w-3/4 md:w-1/2 lg:w-1/3 xl:w-1/4 space-y-4">
          <div className="flex items-center justify-between border-b pb-3">
            <h2 className="text-xl font-semibold text-gray-800">Send OTP</h2>
            <X
              className="cursor-pointer hover:bg-gray-100 rounded transition-colors p-1 text-gray-500 hover:text-gray-800"
              onClick={() => setOtpSendInputOpen(false)}
              size={24}
            />
          </div>
          <form className="space-y-4" onSubmit={onSendOtpSubmit}>
            <div>
              <label
                htmlFor="username"
